...
const MostAverageAssistRanking = () => {
  ...
  async function getRankingData() {
    ...
    axios.get('http://localhost:8000/aramgg/rest_api/ranking/top50_most_avg_assist/')
      .then(res => {
    ...
    <VStack mt="100px" mb="100px">
      ...
      <HStack spacing="40px" mb={10}>
        {rankingData &&
          <>
            ...
            <BronzePodium username={bronzeUserData.username} profile_icon={bronzeUserData.profile_icon} level={bronzeUserData.level} value={roundNumber(bronzeRecord.avg_assist, 2)} unit='Assists/game' />
          </>
        }
      </HStack>
      ...
      { top50Data &&
        <Table w="500px" variant="striped" colorScheme="gray" mb="100px">
          <TableCaption>Average Assists per game Ranking</TableCaption>
          <Thead>
            <Tr>
              <Th>Rank</Th>
              <Th>Summoner Name</Th>
              <Th isNumeric>Avg. number of Assists</Th>
            </Tr>
          </Thead>
            ...
            {top50Data.map((entry, i) => {
              return (
                <Tr>
                  ...
                  <Td isNumeric>{roundNumber(entry.avg_assist, 2)}/game</Td>
                </Tr>
              )
            })}
          </Tbody>
...
export default MostAverageAssistRanking;