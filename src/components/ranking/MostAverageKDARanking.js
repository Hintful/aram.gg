...
const MostAverageKDARanking = () => {
  ...
  async function getRankingData() {
    // ranking data
    axios.get('http://localhost:8000/aramgg/rest_api/ranking/most_avg_kda/')
      .then(res => {
    ...
    axios.get('http://localhost:8000/aramgg/rest_api/ranking/top50_most_avg_kda/')
      .then(res => {
  ...
  return (
    <VStack mt="100px" mb="100px">
      <Text fontFamily="Roboto Condensed" fontSize="24px">⚔️ Average KDA (min. 20 games)</Text>
      <HStack spacing="40px" mb={10}>
        {(rankingData && rankingData[2]) &&
          <>
            <SilverPodium username={silverUserData.username} profile_icon={silverUserData.profile_icon} level={silverUserData.level} value={roundNumber(silverRecord.avg_kda, 2)} unit='KDA' />
            <GoldPodium username={goldUserData.username} profile_icon={goldUserData.profile_icon} level={goldUserData.level} value={roundNumber(goldRecord.avg_kda, 2)} unit='KDA' />
            <BronzePodium username={bronzeUserData.username} profile_icon={bronzeUserData.profile_icon} level={bronzeUserData.level} value={roundNumber(bronzeRecord.avg_kda, 2)} unit='KDA' />
          </>
        }
      </HStack>
      ...
      { top50Data &&
        <Table w="500px" variant="striped" colorScheme="gray" mb="100px">
          <TableCaption>Average KDA per game Ranking</TableCaption>
          <Thead>
            <Tr>
              <Th>Rank</Th>
              <Th>Summoner Name</Th>
              <Th isNumeric>Avg. KDA</Th>
            </Tr>
          </Thead>
            ...
            {top50Data.map((entry, i) => {
              return (
                <Tr>
                  ...
                  <Td isNumeric>{roundNumber(entry.avg_kda, 2)}</Td>
                </Tr>
              )
            })}
          </Tbody>
...
export default MostAverageKDARanking;